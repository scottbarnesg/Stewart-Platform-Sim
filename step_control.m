...
while(run_sim == true)
    ...
    [servo_angles, error_data] = controller_v2(eul_plat_state, actuator_states, trans_plat_state, accel_plat_state, error_data, dt, platform_orientation.time(length(platform_orientation.time)));
...
for i = 1:size(platform_orientation.time, 1)
    ...
    % ax(i) = 1000*(platform_translation_abs.signals.values(i, 1) - platform_translation_abs.signals.values(1, 1));
    % ay(i) = 1000*(platform_translation_abs.signals.values(i, 2) - platform_translation_abs.signals.values(1, 2));
    % az(i) = 1000*(platform_translation_abs.signals.values(i, 3) - platform_translation_abs.signals.values(1, 3));
end
figure;
subplot(3, 1, 1);
